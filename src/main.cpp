...
void print_help(po::options_description &desc, const std::string &argv_0) {
    ...
    // Plugin description
    std::vector<mana::plugin::pIPlugin> plugins =
        mana::plugin::PluginManager::get_instance().get_plugins();

    if (!plugins.empty()) {
        std::cout << "Available plugins:" << std::endl;
        for (const auto &it : plugins) {
            std::cout << "  - " << *it->get_id() << ": " << *it->get_description()
                      << std::endl;
        }
    ...
    std::cout << "  " << filename << " -dresources -dexports -x out/ program.exe"
              << std::endl;
    std::cout << "  " << filename << " --dump=imports,sections --hashes program.exe"
              << std::endl;
    std::cout << "  " << filename << " -r malwares/ --plugins=peid,clamav --dump all"
              << std::endl;
}
 ...
 *
 *	@param	const std::vector<std::string>& args A vector containing the raw
 *program_options arguments.
 *
 ...
 *	@param	po::options_description& desc The description of the arguments (only so it can
 *be passed to print_help if needed).
 *	@param	char** argv The raw arguments (only so it can be passed to print_help if
 *needed).
 *
...
bool validate_args(po::variables_map &vm, po::options_description &desc, char **argv) {
    ...
    if (vm.count("dump")) {
        std::vector<std::string> selected_categories =
            tokenize_args(vm["dump"].as<std::vector<std::string>>());
        const std::vector<std::string> categories = boost::assign::list_of("all")(
            "summary")("dos")("pe")("opt")("sections")("imports")("exports")("resources")(
            "version")("debug")("tls")("config")("delay")("rich");
    ...
    if (vm.count("plugins")) {
        std::vector<std::string> selected_plugins =
            tokenize_args(vm["plugins"].as<std::vector<std::string>>());
        std::vector<mana::plugin::pIPlugin> plugins =
            mana::plugin::PluginManager::get_instance().get_plugins();
        for (const auto &it : selected_plugins) {
            ...
            auto found = std::find_if(
                ...
                boost::bind(&mana::plugin::name_matches, it, boost::placeholders::_1));
    ...
    if (vm.count("output")) {
        ...
        auto found = std::find(formatters.begin(), formatters.end(),
                               vm["output"].as<std::string>());
        if (found == formatters.end()) {
            print_help(desc, argv[0]);
            std::cout << std::endl;
            PRINT_ERROR << "output formatter " << vm["output"].as<std::string>()
...
bool parse_args(po::variables_map &vm, int argc, char **argv) {
    ...
    desc.add_options()("help,h", "Displays this message.")(
        "version,v", "Prints the program's version.")(
        "pe", po::value<std::vector<std::string>>(),
        "The PE to analyze. Also accepted as a positional argument. "
        "Multiple files may be specified.")(
        "recursive,r", "Scan all files in a directory (subdirectories will be ignored).")(
        "output,o", po::value<std::string>(),
        "The output format. May be 'raw' (default) or 'json'.")(
        "dump,d", po::value<std::vector<std::string>>(),
        "Dump PE information. Available choices are any combination of: "
        "all, summary, dos (dos header), pe (pe header), opt (pe optional header), "
        "sections, "
        "imports, exports, resources, version, debug, tls, config (image load "
        "configuration), "
        ...
        "Extract the PE resources and authenticode certificates "
    ...
    try {
        ...
        PRINT_ERROR << "Could not parse the command line (" << e.what() << ")."
                    << std::endl
                    << std::endl;
        return false;
    }
    ...
    if (vm.count("version")) {
        std::stringstream ss;
        ss << "Manalyze " MANALYZE_VERSION
              " (Ivan Kwiatkowski, GPLv3 License) compiled with:"
           << std::endl;
        ss << "* Boost " BOOST_LIB_VERSION " (Boost.org, Boost Software License)"
           << std::endl;
        ss << "* Yara " << YR_MAJOR_VERSION << "." << YR_MINOR_VERSION << "."
           << YR_MICRO_VERSION << ". (Victor M. Alvarez, Apache 2.0 License)"
           << std::endl;
        ss << "* hash-library " << HASH_LIBRARY_VERSION
           << " (Stephan Brumme, ZLib License)." << std::endl;
#if defined WITH_OPENSSL
        ss << "* " << OPENSSL_VERSION_TEXT << " (OpenSSL Project, OpenSSL License)"
           << std::endl;
#endif
        std::cout << ss.str();
        exit(0);
 ...
 *
 *	@param	mana::io::OutputFormatter& formatter The object which will receive the output.
 ...
 *			below.
 *	@param	const mana::pe::PE& pe The PE to dump.
 *	@param	bool compute_hashes Whether hashes should be calculated.
 */
void handle_dump_option(mana::io::OutputFormatter &formatter,
                        const std::vector<std::string> &categories, bool compute_hashes,
                        const mana::pe::PE &pe) {
    bool dump_all =
        (std::find(categories.begin(), categories.end(), "all") != categories.end());
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "summary") != categories.end()) {
        mana::dump_summary(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "dos") != categories.end()) {
        mana::dump_dos_header(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "pe") != categories.end()) {
        mana::dump_pe_header(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "opt") != categories.end()) {
        mana::dump_image_optional_header(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "sections") != categories.end()) {
        mana::dump_section_table(pe, formatter, compute_hashes);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "imports") != categories.end()) {
        mana::dump_imports(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "exports") != categories.end()) {
        mana::dump_exports(pe, formatter);
    }
    if (dump_all || std::find(categories.begin(), categories.end(), "resources") !=
                        categories.end()) {
        mana::dump_resources(pe, formatter, compute_hashes);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "version") != categories.end()) {
        mana::dump_version_info(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "debug") != categories.end()) {
        mana::dump_debug_info(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "tls") != categories.end()) {
        mana::dump_tls(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "config") != categories.end()) {
        mana::dump_config(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "delay") != categories.end()) {
        mana::dump_dldt(pe, formatter);
    }
    if (dump_all ||
        std::find(categories.begin(), categories.end(), "rich") != categories.end()) {
 ...
 *	@param	const mana::pe::PE& pe The PE to analyze.
 */
void handle_plugins_option(mana::io::OutputFormatter &formatter,
                           const std::vector<std::string> &selected, const config &conf,
                           const mana::pe::PE &pe) {
    bool all_plugins =
        std::find(selected.begin(), selected.end(), "all") != selected.end();
    std::vector<mana::plugin::pIPlugin> plugins =
        mana::plugin::PluginManager::get_instance().get_plugins();
    mana::io::pNode plugins_node(
        new mana::io::OutputTreeNode("Plugins", mana::io::OutputTreeNode::LIST));

    typedef std::tuple<boost::shared_ptr<std::thread>,
                       std::shared_future<mana::plugin::pResult>, mana::plugin::pIPlugin>
        plugin_job;
    std::vector<plugin_job> jobs;
    for (const auto &it : plugins) {
        // Verify that the plugin was selected
        if (!all_plugins && std::find(selected.begin(), selected.end(), *it->get_id()) ==
                                selected.end()) {
            continue;
        }
        ...
        std::promise<mana::plugin::pResult> res_promise;
        std::shared_future<mana::plugin::pResult> future(res_promise.get_future());

        boost::shared_ptr<std::thread> t(boost::make_shared<std::thread>(std::thread(
            [](const mana::plugin::pIPlugin &p, std::promise<mana::plugin::pResult> r,
               const mana::pe::PE *pe) { r.set_value(p->analyze(*pe)); },
    ...
    for (auto j : jobs) {
        ...
        if (!res) {
            PRINT_WARNING << "Plugin " << *std::get<2>(j)->get_id()
                          << " returned a NULL result!" << std::endl;
            continue;
        }

        mana::io::pNode output = res->get_output();
    ...
    if (vm.count("recursive")) {
        ...
        for (const auto &it : vect) {
            if (!bfs::is_directory(it)) {
                ...
                PRINT_WARNING << it
                              << " is a directory. Skipping (use the -r option for "
                                 "recursive analyses)."
                              << std::endl;
            }
        }
 ...
 */
void perform_analysis(const std::string &path, po::variables_map &vm,
                      ...
                      const config &conf,
                      boost::shared_ptr<mana::io::OutputFormatter> formatter) {
    mana::pe::PE pe(path);
...
int main(int argc, char **argv) {
    ...
    if (working_dir
            .empty()) {    // cmd.exe does not provide the full path to the executable.
...
// nothing is found in the current folder.
    ...
    mana::plugin::PluginManager::get_instance().load_all(working_dir.string());
    ...
    if (vm.count("output") && vm["output"].as<std::string>() == "json") {
        formatter.reset(new mana::io::JsonFormatter());
    } else // Default: use the human-readable output.
    {
        formatter.reset(new mana::io::RawFormatter());
    ...
    for (const auto &it : targets) {
        perform_analysis(it, vm, extraction_directory, selected_categories,
                         selected_plugins, conf, formatter);
        if (++count % 1000 == 0) {
            formatter->format(
                std::cout, false); // Flush the formatter from time to time, to avoid
                                   // eating up all the RAM when analyzing gigs of files.
        }
    }
    ...
    if (vm.count("plugins")) {
        ...
        mana::plugin::PluginManager::get_instance().unload_all();
    }
