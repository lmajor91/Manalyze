...
#ifndef __MANACOMMONS_OUTPUT_TREE_NODE__
#define __MANACOMMONS_OUTPUT_TREE_NODE__ 1
...
#pragma region fields
  ...
  private:
    ...
    display_modifier _modifier; // Additional info hinting at how the data should be
...
#pragma region methods
  public:
    ...
    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, boost::uint32_t i,
                                        display_modifier mod = DEC)
        : _name(new std::string(name)), _type(UINT32),
          _uint32_data(new boost::optional<boost::uint32_t>(i)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, boost::uint16_t s,
                                        display_modifier mod = DEC)
        : _name(new std::string(name)), _type(UINT16),
          _uint16_data(new boost::optional<boost::uint16_t>(s)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, boost::uint64_t l,
                                        display_modifier mod = DEC)
        : _name(new std::string(name)), _type(UINT64),
          _uint64_data(new boost::optional<boost::uint64_t>(l)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, float f,
                                        display_modifier mod = NONE)
        : _name(new std::string(name)), _type(FLOAT),
          _float_data(new boost::optional<float>(f)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, double d,
                                        display_modifier mod = NONE)
        : _name(new std::string(name)), _type(DOUBLE),
          _double_data(new boost::optional<double>(d)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, const std::string &s,
                                        display_modifier mod = NONE)
        : _name(new std::string(name)), _type(STRING),
          _string_data(new boost::optional<std::string>(s)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, const nodes &n,
                                        display_modifier mod = NONE)
        : _name(new std::string(name)), _type(LIST),
          _list_data(new boost::optional<nodes>(n)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, const strings &strs,
                                        display_modifier mod = NONE)
        : _name(new std::string(name)), _type(STRINGS),
          _strings_data(new boost::optional<strings>(strs)), _modifier(mod) {}

    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, const string_set &strs,
                                        display_modifier mod = AFTER_NAME)
        : _name(new std::string(name)), _type(STRINGS), _modifier(mod) {
        _strings_data = boost::make_shared<boost::optional<strings>>(
     ...
     *	@param	enum display_modifier mod = NONE A modifier to alter the way the node is
     *printed.
     */
    DECLSPEC_MANACOMMONS OutputTreeNode(const std::string &name, enum node_type type,
                                        enum display_modifier mod = NONE);

    // ----------------------------------------------------------------------------

    DECLSPEC_MANACOMMONS pString get_name() const { return _name; }

    // ----------------------------------------------------------------------------

    DECLSPEC_MANACOMMONS void set_name(const std::string &name) { _name->assign(name); }

    // ----------------------------------------------------------------------------

    DECLSPEC_MANACOMMONS node_type get_type() const { return _type; }

    // ----------------------------------------------------------------------------

    DECLSPEC_MANACOMMONS display_modifier get_modifier() const { return _modifier; }

    // ----------------------------------------------------------------------------

    DECLSPEC_MANACOMMONS void set_modifier(display_modifier mod) { _modifier = mod; }
     ...
     *
     *	For instance, a UINT32 node containing 10 and the HEX modifier will be displayed
     *as "0xA". LIST and STRINGS nodes cannot be displayed this way.
     *
