 ...
 */
class Result {
    friend class IPlugin; // Result's constructor should only be called from
                          // IPlugin::make_result().
  ...
  public:
    ...
    /**
     *	@brief	Add any kind of information to a result. This should primarily be used
     *with strings, but some plugins may need to return more complex information.
     *
     *	@param	T t The object to add to the result output.
     */
    template <class T> void add_information(T t) {
        io::pNode output = get_information();
        output->append(boost::make_shared<io::OutputTreeNode>(
            _create_node_name(), t, io::OutputTreeNode::HIDE_NAME));
    }
     ...
     *with strings, but some plugins may need to return more complex information.
     *
     *	Contrary to the previous function, this one allows the user to give a name to the
     *node. This is useful to display STRINGS and LIST nodes properly.
     *
     *	@param	const std::string& name The name to give to the node.
     *	@param	T t The object to add to the result output.
     */
    template <class T> void add_information(const std::string &name, T t) {
        io::pNode output = get_information();
    ...
    /**
     *	@brief	Returns the node of the result's data containing the plugin's "free"
     *output.
     *
     *	@returns	A pointer to a node which is never NULL. The node may be empty
     *however.
     */
    DECLSPEC_MANACOMMONS mana::io::pNode get_information() const;
  ...
  private:
     ...
     *
     *	The name of the node is simply an index, because we do not intend to print it in
     *most cases.
     */
 ...
 */
template <> DECLSPEC_MANACOMMONS void Result::add_information(io::pNode node);

} // namespace mana::plugin

#endif // __MANACOMMONS_PLUGIN_RESULT__
