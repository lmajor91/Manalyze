...
#ifndef __MANAPE_PE__
#define __MANAPE_PE__ 1
...
#pragma region private fields
  private:
    ...
    */
    ...
    std::vector<pcoff_symbol>
        _coff_symbols; // This debug information is parsed (crudely) but
    ...
    std::vector<pimage_base_relocation>
        _relocations; // Not displayed either, because of how big it is.
...
#pragma region public methods
  public:
    DECLSPEC_MANAPE PE(const std::string &path);
    DECLSPEC_MANAPE virtual ~PE() {}
    DECLSPEC_MANAPE static boost::shared_ptr<PE> create(const std::string &path);

    DECLSPEC_MANAPE boost::uint64_t get_filesize() const;

    DECLSPEC_MANAPE pString get_path() const {
     ...
     */
    DECLSPEC_MANAPE shared_sections get_sections() const {
     ...
     */
    DECLSPEC_MANAPE const_shared_strings get_imported_dlls() const;
     ...
     *
     *	@return	A shared vector in which the imported function names will be stored.
     *Functions imported by ordinal will be returned as "#N",  N being the ordinal number.
     *
     *	Implementation is located in imports.cpp.
     */
    DECLSPEC_MANAPE const_shared_strings
    get_imported_functions(const std::string &dll) const;

    /**
     *	@brief	Finds imported DLLs whose names match a particular regular expression.
     *
     *	@param	const std::string& name_regexp The regular expression used to match DLL
     *names.
     *	@param	std::vector<pimage_library_descriptor>& destination The vector into which
     *the result should be stored.
     *	@param	bool case_sensitivity Whether the regular expression should be case
     ...
     *	Implementation is located in imports.cpp.
     */
    DECLSPEC_MANAPE shared_imports find_imported_dlls(
        const std::string &name_regexp, bool case_sensitivity = false) const;

    /**
     *	@brief	Finds imported functions matching regular expressions.
     *
     *	@param	const std::string& function_name_regexp		The regular expression
     *selecting function names.
     *	@param	const std::string& dll_name_regexp			The regular expression
     *selecting imported dlls into which the functions should be searched.
     *	@param	bool case_sensitivity						Whethter the regular
     *expression should be case sensitive (default is false).
     *
     ...
     *	Implementation is located in imports.cpp.
     */
    DECLSPEC_MANAPE const_shared_strings find_imports(
        const std::string &function_name_regexp,
        const std::string &dll_name_regexp = ".*", bool case_sensitivity = false) const;
     ...
     *
     *	@return	The corresponding offset in the file, or 0 if the RVA could not be
     *translated.
     */
    DECLSPEC_MANAPE unsigned int rva_to_offset(boost::uint64_t rva) const;

    DECLSPEC_MANAPE boost::optional<dos_header> get_dos_header() const { return _h_dos; }

    DECLSPEC_MANAPE boost::optional<pe_header> get_pe_header() const { return _h_pe; }

    DECLSPEC_MANAPE boost::optional<image_optional_header>
    get_image_optional_header() const {
        return _ioh;
    }

    DECLSPEC_MANAPE shared_resources get_resources() const {
        return _initialized ? boost::make_shared<std::vector<pResource>>(_resource_table)
                            : shared_resources();
    }

    DECLSPEC_MANAPE shared_exports get_exports() const {
        return _initialized
                   ? boost::make_shared<std::vector<pexported_function>>(_exports)
                   : shared_exports();
    }

    DECLSPEC_MANAPE shared_debug_info get_debug_info() const {
        return _initialized ? boost::make_shared<std::vector<pdebug_directory_entry>>(
                                  _debug_entries)
                            : shared_debug_info();
    }

    DECLSPEC_MANAPE shared_relocations get_relocations() const {
        return _initialized
                   ? boost::make_shared<std::vector<pimage_base_relocation>>(_relocations)
                   : shared_relocations();
    }

    DECLSPEC_MANAPE shared_tls get_tls() const {
        return (_initialized && _tls) ? boost::make_shared<image_tls_directory>(*_tls)
                                      : shared_tls();
    }

    DECLSPEC_MANAPE shared_config get_config() const {
        return (_initialized && _config)
                   ? boost::make_shared<image_load_config_directory>(*_config)
                   : shared_config();
    }

    DECLSPEC_MANAPE shared_dldt get_delay_load_table() const {
        return (_initialized && _delay_load_directory_table)
                   ? boost::make_shared<delay_load_directory_table>(
                         *_delay_load_directory_table)
                   : shared_dldt();
    }

    DECLSPEC_MANAPE shared_certificates get_certificates() const {
        return _initialized
                   ? boost::make_shared<shared_certificates::element_type>(_certificates)
                   : boost::make_shared<shared_certificates::element_type>();
    }

    DECLSPEC_MANAPE shared_rich_header get_rich_header() const {
        return (_initialized && _rich_header)
                   ? boost::make_shared<rich_header>(*_rich_header)
                   : shared_rich_header();
    }

    DECLSPEC_MANAPE shared_imports get_imports() const {
        return (_initialized)
                   ? boost::make_shared<std::vector<pImportedLibrary>>(_imports)
                   : shared_imports();
    }
     ...
     */
    DECLSPEC_MANAPE architecture get_architecture() const;
     ...
     */
    DECLSPEC_MANAPE bool is_valid() const { return _initialized; }
     ...
     */
    DECLSPEC_MANAPE shared_bytes get_raw_bytes(size_t size = INT_MAX) const;
     ...
     */
    DECLSPEC_MANAPE shared_bytes get_overlay_bytes(size_t size = INT_MAX) const;

    /**
     *	@brief	The delete operator. "new" had to be re-implemented in order to make it
     *private.
     *
...
#pragma region private methods
  private:
     ...
     *
     *	Users can't be allowed to allocate objects on the heap themselves. If this happens
     *across DLL boundaries, the heap will get corrupted.
     */
     ...
     * /!\ This relies on the information gathered in _parse_dos_header. Please do not
     * call this function first! Actually, please don't call it at all. Let the
     * constructor handle the parsing.
     */
    ...
    /**
     *	@brief	Courtesy function used to parse all the PE directories (imports, exports,
     *resources, ...).
    ...
    /**
     *	@brief	Translates a Virtual Address (*not relative to the image base*) into an
     *offset in the file.
     *
     *	@param	boost::uint32_t rva The VA to translate
     *
     *	@return	The corresponding offset in the file, or 0 if the VA could not be
     *translated.
     */
     ...
     *	@param	unsigned int offset The offset at which to jump before reading the
     *directory. The offset is relative to the beginning of the resource "section" (NOT a
     *RVA). If it is 0, the function reads from the cursor's current location.
     *
