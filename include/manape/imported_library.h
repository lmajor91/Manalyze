...
#ifndef __MANAPE_IMPORTED_LIBRARY__
#define __MANAPE_IMPORTED_LIBRARY__ 1
...
class ImportedLibrary {
  public:
    ...
    DECLSPEC_MANAPE LOAD_TYPE get_type() const { return _load_type; }
    DECLSPEC_MANAPE pString get_name() const {
        return boost::make_shared<std::string>(_library_name);
    }
    DECLSPEC_MANAPE pImports get_imports() const { return _imported_functions; }
     ...
     *			delay-loaded DLLs.
     */
    DECLSPEC_MANAPE pimage_import_descriptor get_image_import_descriptor() const {
        return _image_import_descriptor;
    }

    void add_import(pimport_lookup_table import) {
        _imported_functions->push_back(import);
    }
...
#endif //__MANAPE_IMPORTED_LIBRARY__
