...
/**
 *	@brief	Abstract class describing objects whose role is to display the output of the
 *program in a specific format (raw, json, ...).
 */
class OutputFormatter {
  public:
    OutputFormatter() { _root = pNode(new OutputTreeNode("root", OutputTreeNode::LIST)); }
     ...
     */
    virtual void set_header(const std::string &header) { _header = header; }

    // ----------------------------------------------------------------------------

    virtual void set_footer(const std::string &footer) { _footer = footer; }
    ...
    void add_data(pNode n, const std::string &file_path) {
        ...
        if (file_node) {
            if (file_node->find_node(*n->get_name())) {
                PRINT_WARNING << "Multiple nodes using the name " << *n->get_name()
                              << " in a dictionary." << std::endl;
            }
            file_node->append(n);
        } else {
     ...
     *	@param	Whether the stream ends here. Set to false if more data will be appended
     *later on.
     *
     *	This last parameter was added because writing the output at the end may cause too
     *much information to be stored in the RAM. Using end_stream enables the caller to
     *flush the formatter's data from time to time.
     */
  ...
  protected:
    std::string _header;
    std::string _footer;
    boost::shared_ptr<OutputTreeNode>
        _root; // The analysis data is contained in this field
};
...
class RawFormatter : public OutputFormatter {
  ...
  private:
     ...
     *
     *	@param	std::stringstream& sink The stringstream into which the data should be
     *written.
     *	@param	pNode node The node to dump.
     *	@param	int max_width The size of the children's biggest name of the parent node
     *(for pretty printing purposes).
     *	@param	int level The hierarchical level of the node to dump (higher is deeper in
     *the tree).
     */
     ...
     *
     *	The plugins' output needs special code to be printed in a more readable fashion
     *than a simple list of keys and values.
     *
     *	@param	std::stringstream& sink The stringstream into which the data should be
     *written.
     *	@param	pNode node The node to dump.
     ...
     *the tree).
     */
    void _dump_strings_node(std::ostream &sink, pNode node, size_t max_width = 0,
                            int level = 0);
};
...
class JsonFormatter : public OutputFormatter {
  ...
  private:
     ...
     *	@param	bool append_comma Whether a comma should be appended at the end of the
     node contents (useful when dumping lists). *	@param	bool print_name Whether the
     name of the node should be displayed (useful when printing *			inside JSON
     lists).
     */
    void _dump_node(std::ostream &sink, pNode node, int level = 1,
                    bool append_comma = false, bool print_name = true);
};
