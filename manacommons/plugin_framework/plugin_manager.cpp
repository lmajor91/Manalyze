...
void PluginManager::load(const std::string &path) {
    ...
    if (!c || !d) {
        ...
        if (libname.find("libplugin_") == 0 || libname.find("plugin_") == 0) {
            PRINT_ERROR << "Could not resolve " << path
                        << "'s creator or destroyer function!" << std::endl;
        }
        return;
    }
    pPlugin plugin = pPlugin(new DynamicPlugin(c, d));

    // Check the api version for dynamic plugins. Over time, the API may evolve and old
    // plugins could become incompatible.
    pIPlugin test = plugin->instantiate_plugin();
    if (test->get_api_version() != API_VERSION) {
        PRINT_ERROR << "The plugin " << *test->get_id()
                    << " is not compatible with this version of the API (expected: "
                    << API_VERSION << ", found: " << test->get_api_version() << ")!"
                    << std::endl;
        return;
    }
    ...
    for (std::vector<pIPlugin>::iterator it = loaded_plugins.begin();
         it != loaded_plugins.end(); ++it) {
        if (*(*it)->get_id() == *test->get_id()) {
            PRINT_WARNING << "The plugin " << *test->get_id() << " tried to load twice!"
                          << std::endl;
            return;
        }
    }
...
void PluginManager::unload_all() { _plugins.clear(); }

bool name_matches(const std::string &s, pIPlugin p) { return *p == s; }
